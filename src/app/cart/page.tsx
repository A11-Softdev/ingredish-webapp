...
function Page() {
  ...
  const fetchCartItems = () => {
    axios
      .get(`${process.env.NEXT_PUBLIC_BASE_URL}/carts/my-cart`, {
        headers: {
  ...
  const handleQuantityChange = (productId: string, newQuantity: number) => {
    ...
    if (typeof newQuantity === "number") {
      axios
        .patch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/carts/my-cart/setProduct`,
          {
            productId,
            amount: newQuantity,
          },
  ...
  const calculateTotal = () => {
    return cartItems.reduce(
      (total, item) => total + item.productId.price * item.amount,
      0
    );
  };
  ...
  return (
    <div className="flex flex-col min-h-[81vh] items-center p-5 w-full">
      <div className="bg-dark-yellow w-2/5 flex justify-center text-4xl font-bold p-4 rounded-t-xl shadow-lg [text-shadow:_0_1px_2px_rgb(0_0_0_/_0.8)]">
        Your Cart
      </div>
      <div className="bg-background w-2/5 flex flex-col gap-4 p-8 rounded-b-xl shadow-xl items-center">
        ...
        <div className="bg-dark-yellow p-4 w-2/5 shadow-lg rounded-lg flex justify-center text-lg font-semibold">
          รวม {calculateTotal()} ฿
        </div>
        <button
          className={`bg-success p-4 shadow-lg rounded-lg flex justify-center font-bold ${
            cartItems.length === 0 ? "opacity-50 cursor-not-allowed" : ""
          }`}
          disabled={cartItems.length === 0} // Disable button if cart is empty
          onClick={() => {
            if (cartItems.length > 0) {
              router.push("/checkout"); // Only navigate if cart has items
            }
          }}
        >
          Check Out
